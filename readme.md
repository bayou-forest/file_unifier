...
2つのフォルダを選択し、含まれるファイルを再帰的にスキャンします。各ファイルのファイルパス・ファイル名・MD5ハッシュ値を取得し、以下9パターンで重複を検出します。
...
| 7 | 名前+Hash重複 (A内) | フォルダA内でファイル名とMD5ハッシュが両方とも同一のファイル |
| 8 | 名前+Hash重複 (B内) | フォルダB内でファイル名とMD5ハッシュが両方とも同一のファイル |
| 9 | 名前+Hash重複 (A↔B) | フォルダA/B間でファイル名とMD5ハッシュが両方とも同一のファイル |
...
- 画面上部にキャッシュ件数が常時表示され、「詳細を表示」ボタンで対象ファイルの一覧（パス・サイズ・MD5）を確認できます
...
現在のバージョンはウィンドウタイトルにも表示されます（例: `File Unifier v1.2.0`）。

| バージョン | 内容 |
|-----------|------|
| 1.2.0 | 「名前+Hash重複」タブ（A内/B内/A↔B）を追加。ハッシュキャッシュの件数表示と詳細一覧（ボタン操作）を追加。ウィンドウタイトルにバージョン表示を追加 |
| 1.1.0 | ユニークファイルのコピー機能（ハッシュ基準）を追加。サムネイルポップオーバーの位置ずれと確認ダイアログの表示崩れを修正 |
| 1.0.0 | 初回リリース。重複ファイル検索（Hash/名前 × A内/B内/A↔B）、フラット化、空フォルダ削除、日付別ファイル分類、サムネイルプレビュー、スキャンの中断・再開（キャッシュ）を実装 |
